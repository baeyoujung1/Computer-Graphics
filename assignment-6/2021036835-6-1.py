...
def render():
    ...
    zalpha = np.radians(alpha)
    xbeta = np.radians(beta)
    zgamma = np.radians(gamma)

    M = np.identity(4)
    Rz1 = np.array([[np.cos(zgamma), -np.sin(zgamma), 0],
                   [np.sin(zgamma), np.cos(zgamma), 0],
                   [0,0,1]])
    Rx = np.array([[1,0,0],
                   [0, np.cos(xbeta), -np.sin(xbeta)],
                   [0, np.sin(xbeta), np.cos(xbeta)]])
    Rz2 = np.array([[np.cos(zalpha), -np.sin(zalpha), 0],
                   [np.sin(zalpha), np.cos(zalpha), 0],
                   [0,0,1]])
    M[:3,:3] = Rz2 @ Rx @ Rz1
    glMultMatrixf(M.T)
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2021036835-6-1', None,None)
    if not window:
        glfw.terminate()
        return
